...
const Checkbox = ({ label, checked, onChange, disabled, stacked }: CheckboxProps) => {
    if (stacked) {
        return (
            <div className="flex flex-col gap-2">                
                <p className={`text-sm align-baseline font-medium text-gray-700 dark:text-gray-300 ${disabled ? 'line-through' : ''}`}>{label}</p>
                <button
